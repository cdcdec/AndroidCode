...
public class PrinterConnectDialog extends AppCompatActivity {
    ...
    private BroadcastReceiver PrinterStatusBroadcastReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (GpCom.ACTION_CONNECT_STATUS.equals(intent.getAction())) {
                ...
                Log.e(DEBUG_TAG, "connect status " + type);
    ...
    void connectOrDisConnectToDevice(int PrinterId) {
        ...
        int rel = 0;
        Log.e(DEBUG_TAG, "getPortOpenState="+String.valueOf(mPortParam[PrinterId].getPortOpenState())+",thread="+Thread.currentThread().getName());
...
//                }
                switch (mPortParam[PrinterId].getPortType()) {
                    case PortParameters.USB:
                        try {
                            Log.e(DEBUG_TAG,"PrinterId="+PrinterId+",type="+mPortParam[PrinterId].getPortType()+",name="+mPortParam[PrinterId].getUsbDeviceName()+",portNum=");
//                            rel = mGpService.openPort(PrinterId, mPortParam[PrinterId].getPortType(),
//                                    mPortParam[PrinterId].getUsbDeviceName(), 0);

                            rel = mGpService.openPort(0, 2,
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (requestCode == INTENT_PORT_SETTINGS) {
            ...
            if (resultCode == RESULT_OK) {
                ...
                Log.e(DEBUG_TAG, "USBDeviceName " + str);
