...
public class ProgressBarCircularIndeterminate extends CustomView {
  ...
  private void k(Canvas paramCanvas) {
    ...
    this.anR.setColor(getResources().getColor(17170445));
    ...
    if (this.dwv >= 50) {
      ...
      float f;
      if (this.dwu >= (getWidth() / 2 - ck.b(3.0F, getResources()))) {
        f = getWidth() / 2.0F - ck.b(3.0F, getResources());
      } else {
        f = 1.0F + this.dwu;
      } 
      this.dwu = f;
  ...
  private void l(Canvas paramCanvas) {
    ...
    this.anR.setColor(getResources().getColor(17170445));
    this.anR.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.CLEAR));
    canvas.drawCircle((getWidth() / 2), (getHeight() / 2), (getWidth() / 2 - ck.b(3.0F, getResources())), this.anR);
    paramCanvas.drawBitmap(bitmap, 0.0F, 0.0F, null);
  }
  
  private void qI() {
    this.anR = new Paint(true);
...
/* Location:              F:\chrome_download\android-Decompile\dex2jar-2.0\classes-dex2jar.jar!\com\laiqia\\ui\ProgressBarCircularIndeterminate.class
