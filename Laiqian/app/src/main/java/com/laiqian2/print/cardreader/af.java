package com.laiqian.print.cardreader;

import com.laiqian.print.model.type.usb.a.a;

class af implements a {
  af(ad paramad) {}
  
  public void a(a parama, boolean paramBoolean) {
    String str = parama.getIdentifier();
    ad.a(this.cEM, str).setConnected(paramBoolean);
    this.cEM.aeL();
  }
}


/* Location:              F:\chrome_download\android-Decompile\dex2jar-2.0\classes2-dex2jar.jar!\com\laiqian\print\cardreader\af.class
 * Java compiler version: 6 (50.0)
 * JD-Core Version:       1.0.4
 */